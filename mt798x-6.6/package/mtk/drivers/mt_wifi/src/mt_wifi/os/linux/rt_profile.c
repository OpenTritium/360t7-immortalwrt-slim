...
static NDIS_STATUS l1set_profile_path(RTMP_ADAPTER *pAd, UINT_32 extra, RTMP_STRING *value)
{
	...
	if (pSemicolon) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "DBDC format of profile path!\n");
		*pSemicolon = '\0';
	}
	...
	if (strcmp(target, value) && (str_len < L2PROFILE_PATH_LEN)) {
		...
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "profile remain %s\n", target);
...
static NDIS_STATUS l1set_ifname(RTMP_ADAPTER *pAd, UINT_32 extra, RTMP_STRING *value)
{
	...
	if (pSemicolon) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "DBDC format of ifname!\n");
		*pSemicolon = '\0';
#ifdef MULTI_PROFILE
	...
	if (target) {
		if (strcmp(target, value)) {
			...
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "ifname remain %s\n", target);
	} else
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		"Default ifname get error! ifname update to %s.\n", value);
		...
		IN RTMP_STRING *chipName)
{
	...
	while (if_idx < MAX_L1PROFILE_INDEX) {
		...
		if (RTMPGetKeyParameter(key, tmpbuf, MAX_PARAM_BUFFER_SIZE, l1profile_data, TRUE)) {
			...
			ret = snprintf(key_slot, 20, "%s_pcie_slot", key);
			...
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				 "key_slot=%s\n", key_slot);
			if (RTMPGetKeyParameter(key_slot, tmpbuf_slot, MAX_PARAM_BUFFER_SIZE, l1profile_data, TRUE)) {
				...
				if ((strncmp(pci_slot, tmpbuf_slot, strlen(pci_slot)) == 0) &&
					...
					if ((strncmp(tmpbuf, chipName, strlen(chipName)) == 0) &&
						...
						ret = snprintf(l1profile[dev_idx].profile_index,
										L1PROFILE_INDEX_LEN, "%s", key);
						if (os_snprintf_error(L1PROFILE_INDEX_LEN, ret)) {
							MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
									 "profile_index snprintf error!!!\n");
						}
			...
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "not find chip name %s\n", key);
		...
		if (RTMPGetKeyParameter(key, tmpbuf, MAX_PARAM_BUFFER_SIZE, l1profile_data, TRUE)) {
			...
			if ((strncmp(tmpbuf, chipName, strlen(chipName)) == 0) && \
				...
				if (is_dup_key(key)) {	/* There might be not only single entry for one chip */
					...
					ret = snprintf(l1profile[dev_idx].profile_index, L1PROFILE_INDEX_LEN, "%s", key);
					if (os_snprintf_error(L1PROFILE_INDEX_LEN, ret)) {
						MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
								 "profile_index snprintf error!!!\n");
					}
			...
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "%s not found, dismissed.\n", key);
...
static NDIS_STATUS l1get_profile_index(IN RTMP_STRING *l1profile_data, IN RTMP_ADAPTER *pAd)
{
	...
	if (IS_MT7615(pAd) && (pAd->RfIcType == RFIC_7615A)) {
		...
		ret = snprintf(chipName + strlen(chipName), sizeof(chipName) - strlen(chipName), "%s", "D");
		if (os_snprintf_error(sizeof(chipName) - strlen(chipName), ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					 "chipName snprintf error!!!\n");
		}
	}
	...
	if (match_index_by_chipname(l1profile_data, pAd, chipName) == NDIS_STATUS_SUCCESS) {
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "[%d]%s found by chip\n", dev_idx, chipName);
	} else {
		retVal = NDIS_STATUS_FAILURE;
...
UCHAR *get_single_sku_path(RTMP_ADAPTER *pAd)
{
	...
	src = l1profile[dev_idx].single_sku_path;
	return src;
}
...
UCHAR *get_bf_sku_path(RTMP_ADAPTER *pAd)
{
	...
	src = l1profile[dev_idx].bf_sku_path;
	return src;
}
...
INT get_dev_config_idx(RTMP_ADAPTER *pAd)
{
	...
	MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "chip_id1=0x%x, chip_id2=0x%x, pAd->MACVersion=0x%x\n", first_card, second_card, pAd->MACVersion);
...
RTMP_STRING *get_dev_eeprom_binary(VOID *pvAd)
{
	...
	return l1profile[dev_idx].ee_info.bin_name;
}
...
UINT32 get_dev_eeprom_offset(VOID *pvAd)
{
	...
	return l1profile[dev_idx].ee_info.offset;
}
...
UINT32 get_dev_eeprom_size(VOID *pvAd)
{
	...
	return l1profile[dev_idx].ee_info.size;
}
...
UCHAR *get_dev_name_prefix(RTMP_ADAPTER *pAd, INT dev_type)
{
	...
	do {
		...
		if (type_idx > MAX_INT_TYPES) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
						"%s(): invalid type_idx(%d)!\n", __func__, type_idx);
			return NULL;
		}
...
UCHAR *get_dev_l2profile(RTMP_ADAPTER *pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if ((dev_idx < MAX_NUM_OF_INF) && (dev_idx >= 0))
...
NDIS_STATUS load_dev_l1profile(IN RTMP_ADAPTER *pAd)
{
	...
	if (srcf.Status) {
		...
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "Open file \"%s\" Succeed!\n", L1_PROFILE_PATH);
	...
	if (retval) {
		...
		if (RTMPGetKeyParameter(profile_index, tmpbuf, MAX_PARAM_BUFFER_SIZE, buffer, TRUE)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "Chip=%s\n", tmpbuf);

			for (attr_index = 0; attr_index < ARRAY_SIZE(l1profile_attributes); attr_index++) {
				l1attr = &l1profile_attributes[attr_index];
				ret = snprintf(key, sizeof(key), "%s_%s", profile_index, l1attr->name);
				if (os_snprintf_error(sizeof(key), ret)) {
...
err_out:
	...
	if ((srcf.Status == 0) && (os_file_close(srcf) != 0)) {
		...
		MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "Close file \"%s\" succeed!\n", L1_PROFILE_PATH);
...
NDIS_STATUS	RTMPReadParametersHook(RTMP_ADAPTER *pAd)
{
...
#endif /*MULTI_PROFILE*/
	{
		...
		if (src && *src) {
			...
			if (srcf.Status)
				MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "Open file \"%s\" failed!\n", src);
			else {
#ifndef OS_ABL_SUPPORT
				...
				if (retval > 0) {
					...
					MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "Read file \"%s\" failed(errCode=%d)!\n", src, retval);

				if (os_file_close(srcf) != 0) {
					retval = NDIS_STATUS_FAILURE;
					MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "Close file \"%s\" failed(errCode=%d)!\n", src, retval);
				}
			}
		}
	}
	...
	IN struct sk_buff		*skb)
{
	...
	if (data == 0) {
		...
		if (data == 0) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "Error\n");
			return 1;
		}
	}
	...
	if (TypeLen ==  ETH_TYPE_ARP)
		...
		if (pData[LayerHdrPos + 9] == 0x11) {
			...
			if (srcPort == 68 && dstPort == 67) { /*It's a DHCP packet */
				...
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_INFO, "is bootp packet! bootpFlag=0x%x\n", bootpFlag);
				dhcpHdr = bootpHdr + 236;

				if (bootpFlag != 0x8000) { /*check if it's a broadcast request. */
					static UCHAR  DHCP_MAGIC[] = {0x63, 0x82, 0x53, 0x63};

					MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_INFO, "the DHCP flag is a unicast, dhcp_magic=%02x:%02x:%02x:%02x\n",
							 dhcpHdr[0], dhcpHdr[1], dhcpHdr[2], dhcpHdr[3]);

					if (NdisEqualMemory(dhcpHdr, DHCP_MAGIC, 4)) {
						MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_MAT, DBG_LVL_INFO, "dhcp magic macthed!\n");
	...
	OUT BOOLEAN *DropPacket)
{
	...
	if (IS_GROUP_MAC(pPktHdr)) {
		...
		if ((pInvalidEntry != NULL) || (pOtherBandReptEntry != NULL) || (pAnotherBandReptEntry != NULL)) {
			MTWF_DBG(NULL, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_DEBUG, "%s, recv broadcast from InvalidRept Entry, drop this packet\n", __func__);
			*DropPacket = TRUE;
		}
	}
...
VOID RTMPFreeAdapter(VOID *pAdSrc)
{
	...
	if ((IS_HIF_TYPE(pAd, HIF_MT)) && (pAd->OpMode == OPMODE_AP)) {
		...
		if (pMbss)
			...
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "func_dev is NULL!\n");
			return;
		}
	}
...
INT RTMP_AP_IoctlPrepare(RTMP_ADAPTER *pAd, VOID *pCB)
{
	...
	if (pConfig->priv_flags == INT_MAIN) {
		...
		if (cap->DbdcOneBand1Support) {
			struct wifi_dev *wdev = pConfig->wdev;

			pObj->ioctl_if = wdev->func_idx;
		} else
#endif
		...
		for (index = 0; index < MAX_WDS_ENTRY; index++) {
			...
			if (index == MAX_WDS_ENTRY) {
				MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "can not find wds I/F\n");
				return -ENETDOWN;
			}
		}
	}
		...
		for (index = 0; index < MAX_APCLI_NUM; index++) {
			...
			if (index == MAX_APCLI_NUM) {
				MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, "can not find Apcli I/F\n");
				return -ENETDOWN;
			}
		}
	...
	IN	RTMP_STRING *msg)
{
	...
	if (wrq->u.data.length >= 4095) {
		MTWF_DBG(NULL, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "%s: length more than iwpriv buffer size 4096"
			", change from lenght:%d to 4095\n  print from driver directly\n\n",
		...
		MTWF_DBG(NULL, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, "%s: copy_to_user() fail, lenght:%d\n",
			__func__, wrq->u.data.length);
}
